...
export class HomePage {
  ...
  ionViewDidLoad() {
    ...
    this.setupScene();
    this.animate();
    let pressGesture = new Gesture(this.canvas.nativeElement);
    pressGesture.listen();

    this.startStream = Observable.fromEvent(pressGesture, "press");
    this.stopStream = Observable.fromEvent(pressGesture, "pressup");

    this.intervalStream = Observable.interval(150).takeUntil(this.stopStream);
    ...
    this.startStream.switchMapTo(this.intervalStream).subscribe(() => {
      this.currPressure++;
    });
